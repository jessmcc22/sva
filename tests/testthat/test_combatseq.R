...
test_that("check ComBat-Seq output with several different parameters on simulated genes",{
  ...
  mod_choice <- evaluate_promise(ComBat_seq(count_matrix, batch, group=rep(1, 8), full_mod=TRUE))
  expect_equal(mod_choice$messages[2], "Using null model in ComBat-seq.\n")
